...
Open the `Local URL` printed by Streamlit (e.g. http://localhost:8501 or :8502) in your browser.
...
- Expand the mock API endpoints to simulate product lists or alternatives for fuller demos.
- Add a toggle in the UI to switch theme palettes (earth/light/dark).
- Add unit tests for `shopimpact/core.py` (simple tests for `record_to_entry()` and `get_multiplier_for_category`).

---

If you want, I can also:
- Add a `requirements.txt` and a small `run.ps1` script that starts the mock API and Streamlit in separate consoles.
- Update the app to call the API status check on load (I can add that to the top of the UI).
